...
declare -A AGENTBOT_COMMAND_USAGE=(
	...
	[workspaces]='agentbot workspaces [--paths0 | --remove PATH]'
...
declare -A AGENTBOT_COMMAND_SUMMARY=(
	...
	[install]='Install skills, synchronize optional Graphify Agent Skills, refresh outputs, run Doctor, and link Agentbot.'
	[update]='Refresh upstream skills, optional Graphify Agent Skills, and managed workspace/global outputs after the repo-first gate.'
	...
	[workspaces]='List locally registered workspaces or stop managing one recorded path.'
...
declare -A AGENTBOT_COMMAND_OPTIONS=(
	...
	[update]=$'--dry-run|Preview reconciliation and managed-surface changes without writing.|off\n--yes|Pre-approve source-owned skill additions, removals, and manifest changes.|off'
	...
	[workspaces]=$'--paths0|Print canonical recorded paths separated by NUL bytes for TUI use.|off\n--remove PATH|Stop managing one recorded workspace. No workspace files are changed.|off'
...
declare -A AGENTBOT_COMMAND_DEFAULTS=(
	...
	[update]='Plain update writes successful reconciliation and managed-surface changes; --dry-run is write-free.'
	...
	[workspaces]='Lists the local registry unless --remove explicitly forgets one path.'
...
declare -A AGENTBOT_COMMAND_EFFECTS=(
	...
	[install]='May install skills, run generic Graphify Agent Skills setup when its CLI exists, render global files, run Doctor, and create the Agentbot link.'
	[update]='May pull/reconcile source-owned skills, run generic Graphify Agent Skills setup when its CLI exists, then refresh registered workspaces and global Codex/Claude outputs; --yes pre-approves source-owned deltas.'
	...
	[workspaces]='Listing and --paths0 are read-only; --remove changes only the private registry record and leaves workspace files unchanged.'
...
declare -A AGENTBOT_COMMAND_RELATED=(
	...
	[workspaces]='Use --remove to stop management without deleting files; use resync to preview or apply managed changes.'
...
declare -A AGENTBOT_BACKEND_USAGE=(
	...
	[workspaces]='./install.sh workspaces [--paths0 | --remove PATH]'
	[resync]='./install.sh resync [--all | PATH ...] [--yes|--dry-run]'
)

declare -A AGENTBOT_BACKEND_SUMMARY=(
	[install]='Run the complete first-time Agentbot installation and synchronize generic Graphify Agent Skills when its CLI exists.'
	...
	[update]='Run repository-first skill reconciliation, synchronize generic Graphify Agent Skills when its CLI exists, then refresh workspaces and global Codex/Claude outputs.'
	...
	[workspaces]='List registered workspaces or remove one private registry record without changing workspace files.'
...
declare -A AGENTBOT_BACKEND_OPTIONS=(
	...
	[workspaces]=$'--paths0|Print canonical recorded paths separated by NUL bytes for TUI use.|off\n--remove PATH|Stop managing one recorded workspace without changing its files.|off'
