...
Machine-level baseline for all agent surfaces managed by Agentbot.
...
- Prefer CLI-friendly, idempotent commands (`./install.sh`, `agentbot`).
...
- **Cursor / Copilot** — use globally installed skills and repo `AGENTS.md`
  overlays scaffolded by `agentbot boot`.

## Project / Repo Conventions

- `AGENTBOT_HOME` points at the `agent_bootstrap` clone (set by
  `install.sh`).
- Per-repo policy: scaffold with `agentbot boot`, then edit the `## Project`
